		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						<form action="{{ route('admin.update-sim',$sim->id) }}" method="post" class="row g-3">
							<input type="hidden" name="_token" value="{{ csrf_token() }}">
							<div class="col-md-6">
								<label class="form-label">Số Sim*</label>
								<input type="text" name="phone_number" value="{{ $sim->phone_number }}" class="form-control" id="inputEmail4">
							@error('phone_number')
							...
							<div class="col-md-6">
								...
								<input type="number" name="price" value="{{ $sim->price }}" class="form-control" id="inputPassword4">
							@error('price')
								<p class="text-danger">{{ $message }}</p>
							@enderror
							</div>
							<div class="col-md-6">
								...
								<select id="inputState" name="network_sim" class="form-select">
									...
									<option value="vietnamobile">Sim vietnamobile</option>
									@elseif($sim->network_sim=='vina')
									<option value="viettel">Sim viettel</option>
									<option selected value="vina">Sim vinaphone</option>
									<option value="mobi">Sim mobifone</option>
									<option value="vietnamobile">Sim vietnamobile</option>
									@elseif($sim->network_sim=='mobi')
									<option value="viettel">Sim viettel</option>
									<option value="vina">Sim vinaphone</option>
									<option selected value="mobi">Sim mobifone</option>
									<option value="vietnamobile">Sim vietnamobile</option>
									@elseif($sim->network_sim=='vietnamobile')
									<option value="viettel">Sim viettel</option>
									<option value="vina">Sim vinaphone</option>
									<option value="mobi">Sim mobifone</option>
									<option selected value="vietnamobile">Sim vietnamobile</option>
									@endif
								</select>
							</div>
							<div class="col-md-6">
								...
								<select id="inputState" name="id_cate_sim" class="form-select">
									...
									<option <?php if ($cateSim->id==$sim->id_cate_sim):?>
											selected
											<?php endif?>
										value="{{$cateSim->id}}">{{ $cateSim->name }}</option>
									@endforeach
								</select>
							</div>
							<div class="col-12">
